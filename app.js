...
class UltimateFMApp {
  ...
  renderTickets() {
    ...
    if (techList) {
      ...
      const activeTechTks = allTechTks.filter(tk => ['قيد التنفيذ', 'In Progress', 'تم التعيين للفني', 'جاري العمل', 'انتظار دفع المالك', 'تم الدفع - جاري التركيب', 'قيد الفحص الميداني', 'جديد'].includes(tk.status));
  ...
  assignTechnician(ticketId, selectId) {
    ...
    if (tk) {
      ...
      this.syncTicketUpdateToOdoo(tk).catch(e => console.warn('[Assign Odoo Sync Error]:', e));
    }
  }
