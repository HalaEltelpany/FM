...
class UltimateFMApp {
  ...
  async syncTicketUpdateToOdoo(ticket) {
    ...
    if (!urlInput || !dbInput || !userInput || !keyInput) return;
    const baseUrl = urlInput.replace(/\/+$/, '');
    const uid = 2; // Direct cached admin UID for instant < 500ms execution
    ...
    try {
      const targetStageId = this.resolveOdooStageId(ticket.status, ticket.odooModel);
      console.log(`[Odoo Update] Updating model: ${ticket.odooModel}, ID: ${ticket.odooId}, Target Stage ID: ${targetStageId}`);

      // 1. FAST DIRECT WRITE FIRST (Ensures Stage moves to In Progress / Solved instantly)
      const writeFields = {
      ...
      const writeData = await this.callOdoo(baseUrl, writePayload, 6000);
      if (writeData && writeData.result) {
        console.log(`[Odoo Stage Update Success] Stage updated to ${targetStageId}:`, writeData.result);
        ...
        // If solved (Stage 4 or 3), also close Field Service task (project.task -> stage 19 = Done)
        if ((targetStageId === 4 || targetStageId === 3) && (ticket.fieldServiceTaskId || ticket.odooTaskId)) {
          const fTaskId = ticket.fieldServiceTaskId || ticket.odooTaskId;
          const taskDonePayload = {
            ...
            params: {
              ...
              args: [
                ...
                "project.task",
                "write",
                [[parseInt(fTaskId)], { stage_id: 19 }]
              ]
            },
            id: Math.floor(Math.random() * 1000)
          };
          this.callOdoo(baseUrl, taskDonePayload, 4000).catch(e => console.warn('[Field Service Task Close Failed]:', e));
        }
      } else if (writeData && writeData.error) {
        console.error('[Odoo Stage Update Error]:', writeData.error);
      }

      // 2. Post to Chatter (message_post) asynchronously in background
      ...
      this.callOdoo(baseUrl, chatterPayload, 4000).catch(e => console.warn('[Odoo Chatter Post Failed]', e));
      ...
      if (ticket.photoAfter && ticket.photoAfter.startsWith('data:image')) {
        try {
          ...
          if (base64Content) {
            ...
            this.callOdoo(baseUrl, attachPayload, 6000).catch(e => console.warn('[Odoo Photo Post Failed]', e));
          }
        } catch (photoErr) {
          console.warn('[Odoo Photo After Error]:', photoErr);
        }
      }
    } catch (err) {
      console.error('[Odoo Sync Update Error]:', err);
    }
  }
