  ...
  async syncTicketUpdateToOdoo(ticket) {
    ...
    if (!ticket.odooId) {
      try {
        ...
        const searchRes = await this.callOdoo(creds.baseUrl, searchPayload, 3500);
        if (searchRes && searchRes.result && searchRes.result.length > 0) {
          ticket.odooId = searchRes.result[0].id;
          ticket.odooModel = ticket.odooModel || 'helpdesk.ticket';
          this.saveTicketsToStorage();
        }
      } catch (sErr) {
    ...
    if (!ticket.odooId) {
      console.warn('[Odoo Update] Ticket odooId not found for', ticket.id);
      return false;
    }
