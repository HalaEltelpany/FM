...
class UltimateFMApp {
    ...
    try {
      ...
      if (readData && readData.result && Array.isArray(readData.result)) {
        ...
        readData.result.forEach(rec => {
          ...
          let stageName = Array.isArray(rec.stage_id) ? rec.stage_id[1] : 'جديد';
          if (stageName === 'In Progress') stageName = 'قيد التنفيذ';
          else if (stageName === 'Solved') stageName = 'تم الحل';
          ...
          const existingMatchingLocal = this.tickets.find(t => !t.odooId && (
            ...
            (t.id && cleanName.includes(String(t.id).toLowerCase()))
          ));
  ...
  getOdooCredentials() {
    const url = document.getElementById('odooUrlInput')?.value || safeStorage.getItem('odoo_url') || 'https://edu-fm-uc.odoo.com';
    const db = document.getElementById('odooDbInput')?.value || safeStorage.getItem('odoo_db') || 'edu-fm-uc';
    const user = document.getElementById('odooUserInput')?.value || safeStorage.getItem('odoo_user') || 'fmhala6@gmail.com';
    const key = document.getElementById('odooKeyInput')?.value || safeStorage.getItem('odoo_key') || '06d7d7d208a8c2fa351c2a5cfa305e987ffb72f0';
    return {
      baseUrl: (url || 'https://edu-fm-uc.odoo.com').replace(/\/+$/, ''),
      db: db || 'edu-fm-uc',
      user: user || 'fmhala6@gmail.com',
      key: key || '06d7d7d208a8c2fa351c2a5cfa305e987ffb72f0',
      uid: 2
    };
  }
  ...
  async callOdoo(baseUrl, payload, timeoutMs = 8000) {
    ...
    const cloudflareProxy = 'https://odoo-proxy.fmhala6.workers.dev/';
    const localProxy = 'http://localhost:3100/odoo-proxy';
    const payloadStr = JSON.stringify(payload);

    // 1. Try Cloudflare Worker (works on HTTPS/GitHub Pages - always first)
    try {
      ...
      if (response && response.ok) {
        ...
        if (data) {
          console.log('[Odoo] ✅ Cloudflare Worker succeeded');
          return data;
        }
      }
    } catch (cfErr) {
      console.warn('[Odoo] Cloudflare Worker failed, trying local proxy:', cfErr.message || cfErr);
    }

    // 2. Fallback: local proxy (localhost:3100 via OdooProxy.exe - works on file:// / http)
    try {
      const controller2 = new AbortController();
      const timeoutId2 = setTimeout(() => controller2.abort(), 3000);
      const response2 = await fetch(localProxy, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: payloadStr,
      ...
      if (response2 && response2.ok) {
        ...
        if (data2) {
          console.log('[Odoo] ✅ Local proxy succeeded');
          return data2;
        }
      }
    } catch (localErr) {
      console.warn('[Odoo] Local proxy not available:', localErr.message || localErr);
    }

    // 3. Last resort: direct call
    try {
      ...
      if (response3 && response3.ok) {
        ...
        if (data3) {
          console.log('[Odoo] ✅ Direct call succeeded');
          return data3;
        }
      }
    } catch (directErr) {
  ...
  handleNewTicketSubmit() {
    ...
    setTimeout(() => { this._isTicketSubmitting = false; }, 1500);
    ...
    const fallbacks = {
      ...
      'أعمال مدنية': 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=300&q=80'
    };
    ...
    const proceedWithTicket = (finalPhoto) => {
      ...
      this.showToast(`✅ تم تقديم بلاغ الصيانة بنجاح برقم #${newTicket.id}!\nجاري الحفظ والمزامنة مع أودو...`);

      // Fast background live sync to Odoo
      newTicket._createPromise = this.syncTicketToOdoo(newTicket, '01223456789', 'حسن عيسى').then(() => {
        this.saveTicketsToStorage();
        this.renderTickets();
      }).catch(e => console.warn('[Odoo Initial Sync Warning]:', e));
    };
  ...
  async syncTicketToOdoo(ticket, overridePhone, overrideName) {
    ...
    try {
      ...
      let partnerId = 7755; // حسن عيسى (مالك الوحدة فيلا A104)
      if (ticket.requester === 'tenant') {
        partnerId = 7758; // أحمد زاهر محمود (المستأجر - شاليه A112)
      } else if (ticket.requester === 'commercial') {
        partnerId = 7759; // مطعم وكافيه Sea Breeze (المحل التجاري)
      } else if (ticket.requester === 'manager' || ticket.requester === 'engineer') {
        partnerId = 3; // إدارة المرافق العامة
      }
  ...
  resolveOdooStageId(status, model = 'helpdesk.ticket') {
    if (!status) return 1;
    ...
    const isMaintenance = model === 'maintenance.request';

    // 1. Solved / Completed (Stage 4 in Odoo Helpdesk, Stage 3 Repaired in Maintenance)
    if (s.includes('تم الانتهاء') || s.includes('تم الحل') || s.includes('تم الاغلاق') || s.includes('مكتمل') || s.includes('منتهي') || s.includes('solved') || s.includes('completed') || s.includes('closed') || s.includes('done') || s.includes('repaired')) {
      return isMaintenance ? 3 : 4;
    }
    // 2. Cancelled (Stage 5 in Odoo Helpdesk, Stage 4 Scrap in Maintenance)
    if (s.includes('ملغي') || s.includes('الغاء') || s.includes('cancel') || s.includes('scrap')) {
      return isMaintenance ? 4 : 5;
    }
    // 3. In Progress (Stage 2 in Odoo Helpdesk & Maintenance) - Immediately when assigned or work begins
    if (s.includes('تنفيذ') || s.includes('اسناد') || s.includes('تعيين') || s.includes('جاري') || s.includes('معاين') || s.includes('فني') || s.includes('موقع') || s.includes('دفع') || s.includes('تركيب') || s.includes('سداد') || s.includes('صرف') || s.includes('موافق') || s.includes('progress') || s.includes('assigned')) {
      return 2;
    }
    // 4. New (Stage 1 in Odoo Helpdesk & Maintenance)
  ...
  async syncTicketUpdateToOdoo(ticket) {
    ...
    const creds = this.getOdooCredentials();
    const uid = creds.uid;

    // 1. Wait for creation promise if currently in flight
    if (ticket._createPromise) {
      try {
        await ticket._createPromise;
      } catch (e) {
        console.warn('[Odoo Update] Awaiting create promise failed:', e);
      }
    }

    // 2. If odooId is still missing, search Odoo by ticket ID in name (both models)
    if (!ticket.odooId && String(ticket.id).startsWith('TK-OD-')) {
      ticket.odooId = parseInt(String(ticket.id).replace('TK-OD-', ''));
    }
    if (!ticket.odooId) {
      try {
        const searchModels = [ticket.odooModel || 'helpdesk.ticket', 'helpdesk.ticket', 'maintenance.request'];
        const uniqueModels = [...new Set(searchModels)];
        for (const searchModel of uniqueModels) {
          const searchPayload = {
            jsonrpc: "2.0",
            method: "call",
            params: {
              ...
              method: "execute_kw",
              args: [
                creds.db, uid, creds.key,
                searchModel,
                "search_read",
                [[["name", "ilike", String(ticket.id)]]],
                { fields: ["id", "name", "stage_id"], limit: 1 }
              ]
            },
            id: Math.floor(Math.random() * 1000)
          };
          const searchRes = await this.callOdoo(creds.baseUrl, searchPayload, 8000);
          if (searchRes && searchRes.result && searchRes.result.length > 0) {
            ticket.odooId = searchRes.result[0].id;
            ticket.odooModel = searchModel;
            console.log(`[Odoo Update] Found ticket in [${searchModel}] ID:`, ticket.odooId);
            this.saveTicketsToStorage();
            break;
          }
        }
    ...
    if (!ticket.odooId) {
      console.warn('[Odoo Update] Ticket odooId not found for', ticket.id, '- cannot update Odoo stage');
      return false;
    }


    const odooIdNum = parseInt(String(ticket.odooId).replace(/\D+/g, ''));
    if (!odooIdNum) return false;

    const model = ticket.odooModel || 'helpdesk.ticket';
    const targetStageId = this.resolveOdooStageId(ticket.status, model);

    // 3. Fast Direct Write to update stage and technician
    const techUserMap = {
      'كريم حسن': 6,
      'إبراهيم فؤاد': 7,
      'محمود الشناوي': 8,
    ...
    const writeFields = { stage_id: targetStageId };
    if (ticket.assignedTech) {
      writeFields.user_id = techUserMap[ticket.assignedTech] || 6;
    }
    ...
    try {
      const writeRes = await this.callOdoo(creds.baseUrl, writePayload, 5000);
      if (writeRes && writeRes.result) {
        console.log(`[Odoo Success] Ticket #${odooIdNum} updated to Stage ${targetStageId}`);
        const stageLabel = targetStageId === 2 ? 'قيد التنفيذ (In Progress)' : ((targetStageId === 4 || targetStageId === 3) ? 'تم الحل (Solved)' : 'محدث');
        this.showToast(`🔄 تم تحديث مرحلة البلاغ في أودو بنجاح (${stageLabel})`);

        // If solved (Stage 4 or 3), also close Field Service task if linked
        if ((targetStageId === 4 || targetStageId === 3) && (ticket.fieldServiceTaskId || ticket.odooTaskId)) {
          const fTaskId = parseInt(String(ticket.fieldServiceTaskId || ticket.odooTaskId).replace(/\D+/g, ''));
          if (fTaskId) {
            const taskDonePayload = {
              ...
              params: {
                service: "object",
                method: "execute_kw",
                args: [creds.db, uid, creds.key, "project.task", "write", [[fTaskId], { stage_id: 19 }]]
              },
              id: Math.floor(Math.random() * 1000)
            };
            this.callOdoo(creds.baseUrl, taskDonePayload, 3000).catch(() => {});
          }
        }
        return true;
      }
    } catch (wErr) {
      console.warn('[Odoo Write Error]:', wErr);
    }
    return false;
  }
  ...
  renderTickets() {
    ...
    const getTicketHtml = (tk) => {
      ...
      if (tk.status === 'بانتظار موافقة العميل على القطعة') {
        ...
        `;
      } else if (tk.status === 'تم السداد - بانتظار صرف القطعة من المستودع' || tk.status === 'تم الدفع - جاري التركيب' || tk.status === 'تم صرف القطعة - جاري التركيب') {
        paymentHtml = isEn ? `
          ...
          if (isCancelled) {
            ...
            actionHtml = `
              <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(32,39,79,0.06); display: flex; gap: 8px; align-items: center;">
                ${(() => {
                  const techOptions = [
                    { val: 'إبراهيم فؤاد', text: '🚪 إبراهيم فؤاد (نجارة وأبواب وديكور)', match: 'نجار' },
                    { val: 'محمود الشناوي', text: '🧱 محمود الشناوي (أعمال مدنية وترميمات)', match: 'مدني' },
                    { val: 'مينا جرجس', text: '🔧 مينا جرجس (شبكات وسباكة)', match: 'سباك' },
                    { val: 'أحمد علي', text: '⚡ أحمد علي (كهرباء وطاقة)', match: 'كهرب' },
                    { val: 'كريم حسن', text: '❄️ كريم حسن (تكييف وكهروميكانيك)', match: 'تكييف' },
                    { val: 'سمير عبد الرحيم', text: '🏠 سمير عبد الرحيم (صيانة عامة وأجهزة)', match: 'عام' },
                    { val: 'سعيد محمود', text: '🌿 سعيد محمود (مرافق عامة وتشغيل)', match: 'مرافق' }
                  ];
                  const catStr = (tk.category || '').toLowerCase();
                  let selectedTech = 'كريم حسن';
                  for (const t of techOptions) {
                    if (catStr.includes(t.match)) {
                      selectedTech = t.val;
                      break;
                    }
                  }
                  return `
                    <select class="form-control" style="flex: 1; height: 38px; padding: 0 10px; font-size: 0.76rem; background: #f8fafc; color: #1e293b; border: 1px solid #e2e8f0; font-weight: 700; border-radius: 8px;" id="assignTechSelect_${tk.id}">
                      ${techOptions.map(t => `<option value="${t.val}" ${t.val === selectedTech ? 'selected' : ''}>${t.text}</option>`).join('')}
                    </select>
                  `;
                })()}
    ...
    if (techList) {
      ...
      const allTechTks = this.tickets.filter(tk => (tk.assignedTech || (!tk.assignedTech && tk.requester === 'manager')) && isPureMaintenanceTask(tk));
      const activeTechTks = allTechTks.filter(tk => ['قيد التنفيذ', 'In Progress', 'تم التعيين للفني', 'جاري العمل', 'انتظار موافقة وسداد العميل', 'انتظار دفع المالك', 'تم السداد - بانتظار صرف القطعة من المستودع', 'تم الدفع - جاري التركيب', 'تم صرف القطعة - جاري التركيب', 'قيد الفحص الميداني', 'جديد'].includes(tk.status));
      ...
      if (filteredTechTks.length === 0) {
        ...
        filteredTechTks.forEach(tk => {
          ...
          if (isDone) {
            ...
            `;
          } else if (tk.status === 'تم التعيين للفني' || tk.status === 'جاري العمل' || tk.status === 'جديد' || tk.status === 'قيد الفحص الميداني' || tk.status === 'جاري العمل الميداني' || tk.status === 'قيد التنفيذ') {
            const activePath = tk._techActionPath || 'no_part';
            innerTechHtml = `
              <!-- Status & Mode Switcher -->
              <div style="margin-top: 8px; padding: 6px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px;">
                <div style="font-size: 0.7rem; font-weight: 800; color: #334155; margin-bottom: 6px; display: flex; align-items: center; justify-content: space-between;">
                  <span><i class="fa-solid fa-wrench" style="color: #1b8f91;"></i> مسار تنفيذ البلاغ الميداني:</span>
                  <span style="font-size: 0.62rem; color: #0284c7; background: #e0f2fe; padding: 2px 6px; border-radius: 4px;">قيد التنفيذ</span>
                </div>
                <div style="display: flex; gap: 6px;">
                  <button type="button" style="flex: 1; height: 34px; font-size: 0.72rem; font-weight: 800; border-radius: 6px; cursor: pointer; border: 1px solid ${activePath === 'no_part' ? '#10b981' : '#cbd5e1'}; background: ${activePath === 'no_part' ? '#ecfdf5' : '#ffffff'}; color: ${activePath === 'no_part' ? '#065f46' : '#64748b'};" onclick="app.setTechActionPath('${tk.id}', 'no_part')">
                    <i class="fa-solid fa-circle-check"></i> ١. حل بدون قطعة غيار
                  </button>
                  <button type="button" style="flex: 1.1; height: 34px; font-size: 0.72rem; font-weight: 800; border-radius: 6px; cursor: pointer; border: 1px solid ${activePath === 'need_part' ? '#0284c7' : '#cbd5e1'}; background: ${activePath === 'need_part' ? '#f0f9ff' : '#ffffff'}; color: ${activePath === 'need_part' ? '#0369a1' : '#64748b'};" onclick="app.setTechActionPath('${tk.id}', 'need_part')">
                    <i class="fa-solid fa-boxes-stacked"></i> ٢. يحتاج قطعة غيار (Field Service)
                  </button>
                </div>
              </div>

              ${activePath === 'no_part' ? `
                <!-- Path A: Resolved directly without spare part -->
                <div style="background: #ecfdf5; border: 1px solid #a7f3d0; padding: 10px; border-radius: 8px; margin-top: 6px;">
                  <div style="font-size: 0.72rem; font-weight: 700; color: #065f46; margin-bottom: 6px;">
                    <i class="fa-solid fa-circle-info"></i> تم فحص ومعاينة العطل في الموقع ويمكن إنهاؤه مباشرة دون استهلاك أي قطع غيار:
                  </div>
                  <div style="display: flex; gap: 6px; align-items: center;">
                    <label for="techPhotoAfter_${tk.id}" style="flex: 1; height: 38px; display: flex; align-items: center; justify-content: center; gap: 4px; background: #ffffff; border: 1px dashed #10b981; border-radius: 8px; color: #065f46; font-size: 0.7rem; font-weight: 700; cursor: pointer; margin: 0;">
                    ...
                    <button type="button" style="flex: 1.4; height: 38px; font-size: 0.76rem; font-weight: 800; background: #059669; border: none; border-radius: 8px; color: #ffffff; display: flex; align-items: center; justify-content: center; gap: 6px; margin: 0; cursor: pointer; box-shadow: 0 2px 6px rgba(5,150,105,0.25);" onclick="app.completeTicket('${tk.id}', 'techPhotoAfter_${tk.id}')">
                      <i class="fa-solid fa-check-double"></i> إنهاء وحل البلاغ (Solved)
                    </button>
                  </div>
                </div>
              ` : `
            ...
            innerTechHtml = `
              <div style="background: #f0fdf4; border: 1.5px solid #86efac; padding: 10px 12px; border-radius: 8px; font-size: 0.74rem; color: #166534; margin-top: 6px;">
                <div style="font-weight: 800; margin-bottom: 4px;">
                  <i class="fa-solid fa-screwdriver-wrench" style="color: #16a34a;"></i> تم استلام [${tk.partName || 'قطعة الغيار'}] من المستودع وجاري التركيب بالوحدة!
                </div>
                <div style="font-size: 0.7rem; color: #15803d; margin-bottom: 8px;">
                  بعد الانتهاء من التركيب واختبار التشغيل، التقط صورة الإصلاح واضغط إغلاق التذكرة لإقفالها على Helpdesk و Field Service معاً:
                </div>
                <div style="margin: 6px 0;">
                  <label for="techPhotoAfter_${tk.id}" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 8px 10px; background: #ffffff; border: 1.5px dashed #16a34a; border-radius: 8px; color: #166534; font-size: 0.74rem; font-weight: 700; cursor: pointer;">
                    <i class="fa-solid fa-camera" style="color: #16a34a; font-size: 0.95rem;"></i> 
                    <span id="techPhotoAfterLabel_${tk.id}">📷 صورة بعد التركيب والإصلاح</span>
                  </label>
  ...
  updateManagerTechsBySpecialty() {
    ...
    const techDatabase = {
      ...
      'أعمال مدنية': [
        { name: 'محمود الشناوي', title: 'فني أعمال مدنية وترميمات ومحارة', status: 'متاح الآن 🟢', available: true },
        { name: 'عمر فاروق', title: 'فني بلاط ودهانات وتشطيبات', status: 'متاح الآن 🟢', available: true }
      ],
      'نجارة': [
        { name: 'إبراهيم فؤاد', title: 'فني أول نجارة وأبواب وديكور', status: 'متاح الآن 🟢', available: true },
  ...
  async dispatchToFieldService(ticketId, selectId) {
    ...
    try {
      ...
      const partName = tk.partName || tk.title || 'أعمال صيانة ميدانية';
      const partPrice = tk.partPrice || 0;
      const taskFields = {
  ...
  setTechActionPath(ticketId, path) {
    const tk = this.tickets.find(t => String(t.id) === String(ticketId) || String(t.odooId) === String(ticketId));
    if (tk) {
      tk._techActionPath = path;
      this.renderTickets();
    }
  }

  async requestSparePartFieldService(ticketId) {
    const tk = this.tickets.find(t => String(t.id) === String(ticketId) || String(t.odooId) === String(ticketId));
    if (!tk) return;

    const select = document.getElementById(`fsPartSelect_${tk.id}`);
    const qtyInput = document.getElementById(`fsPartQty_${tk.id}`);
    const partName = select ? select.value : 'محبس نحاس 3/4 بوصة';
    const qty = qtyInput ? parseInt(qtyInput.value) || 1 : 1;

    const invItem = (this.inventoryItems || []).find(i => i.name === partName);
    const partPrice = invItem ? (invItem.price * qty) : 250;
    const techName = tk.assignedTech || 'كريم حسن';

    tk.partName = partName;
    tk.partPrice = partPrice;
    tk.partQty = qty;
    tk.needsPart = true;
    tk.isFieldService = true;
    tk.status = 'انتظار موافقة وسداد العميل';
    tk.bgClass = 'badge-warning';

    this.saveTicketsToStorage();
    this.renderTickets();
    this.showToast(`🚀 تم تسجيل الحاجة لقطعة غيار [${partName}] بقيمة (${partPrice} ج.م)!\nجاري إنشاء أمر الخدمة الميدانية (Field Service) في أودو وإرسال إشعار السداد للمالك...`);

    // Create project.task in Odoo Project 2 (Field Service), Stage 18 (In Progress)
    try {
      const urlInput = safeStorage.getItem('odoo_url') || 'https://edu-fm-uc.odoo.com';
      const dbInput = safeStorage.getItem('odoo_db') || 'edu-fm-uc';
      const keyInput = safeStorage.getItem('odoo_key') || '06d7d7d208a8c2fa351c2a5cfa305e987ffb72f0';
      ...
      let partnerId = (tk.requester === 'tenant') ? 7758 : 7755;
      const unitStr = tk.location || (tk.requester === 'tenant' ? 'شقة A112' : 'فيلا A104');

      const taskFields = {
        name: `[أمر صيانة ميدانية - قطعة غيار] ${partName} (${unitStr})`,
        project_id: 2,
        stage_id: 18,
        partner_id: partnerId,
        description: `<p><b>طلب صيانة ميدانية لتركيب قطعة غيار:</b></p>` +
                     `<p>• <b>القطعة المطلوبة:</b> ${partName} (الكمية: ${qty} - القيمة: ${partPrice} ج.م)</p>` +
                     `<p>• <b>الوحدة:</b> ${unitStr}</p>` +
                     `<p>• <b>الفني المعاين:</b> ${techName}</p>` +
                     `<p>• <b>التذكرة الأصلية:</b> #${tk.id} - ${tk.title}</p>`
      };
      ...
      const res = await this.callOdoo(baseUrl, taskPayload, 8000);
      if (res && res.result) {
        tk.fieldServiceTaskId = res.result;
        tk.odooTaskId = res.result;
        this.saveTicketsToStorage();
        console.log('[Field Service] Live project.task created ID:', res.result);

        // Post chatter on helpdesk ticket
        if (tk.odooId) {
          const chatterPayload = {
            jsonrpc: "2.0",
            method: "call",
            params: {
              ...
              args: [
                dbInput, uid, keyInput, "helpdesk.ticket", "message_post",
                [[parseInt(tk.odooId)]],
                { body: `<p>⚡ <b>أكد الفني (${techName}) بعد المعاينة الحاجة لقطعة غيار [${partName}] بقيمة (${partPrice} ج.م)</b></p><p>تم تحويل البلاغ لأمر خدمة ميدانية بمشروع Field Service برقم (#WO-${res.result}) وبانتظار موافقة وسداد العميل.</p>` }
              ]
            },
            id: Math.floor(Math.random() * 1000)
          };
          this.callOdoo(baseUrl, chatterPayload, 4000).catch(e => console.warn(e));
        }

        this.showToast(`✅ تم تحويل البلاغ بنجاح لأمر صيانة ميدانية (#WO-${res.result}) في نظام Field Service بأودو!`);
      }
    } catch (err) {
      console.warn('[Field Service Task Creation Error]:', err);
    }
  }

  confirmWarehousePickup(ticketId) {
    const tk = this.tickets.find(t => String(t.id) === String(ticketId) || String(t.odooId) === String(ticketId));
    if (!tk) return;

    // Deduct quantity from inventory
    const invItem = (this.inventoryItems || []).find(i => i.name === tk.partName);
    if (invItem) {
      invItem.qty = Math.max(0, invItem.qty - (tk.partQty || 1));
    }

    tk.partsUsed = tk.partsUsed || [];
    tk.partsUsed.push({
      name: tk.partName || 'قطعة غيار',
      qty: tk.partQty || 1,
      price: tk.partPrice || 0,
      warehouse: 'residential',
      date: new Date().toLocaleTimeString('ar-EG')
    });

    tk.status = 'تم صرف القطعة - جاري التركيب';
    tk.bgClass = 'badge-info';
    this.saveTicketsToStorage();
    this.renderTickets();

    this.showToast(`📦 تم توثيق استلام قطعة الغيار [${tk.partName || 'القطعة'}] من المخزن بنجاح!\nالفني في طريقه لتركيب القطعة واختبارها لدى العميل.`);

    // Chatter update on Field Service task in Odoo
      ...
      const chatterPayload = {
        ...
        params: {
          ...
          args: [
            ...
            [[parseInt(taskId)]],
            { body: `<p>📦 <b>قام الفني (${tk.assignedTech || 'كريم حسن'}) باستلام قطعة الغيار [${tk.partName || 'القطعة'}] من مستودع القرية وجاري التوجه لوحدة العميل للتركيب.</b></p>` }
          ]
        },
        id: Math.floor(Math.random() * 1000)
      };
  ...
  clearClientSpareSig() {
    const canvas = document.getElementById('clientSpareSigCanvas');
    if (canvas) {
      const ctx = canvas.getContext('2d');
  ...
  initClientSpareSigCanvas() {
    const canvas = document.getElementById('clientSpareSigCanvas');
    if (!canvas || canvas._initialized) return;
    canvas._initialized = true;

    const ctx = canvas.getContext('2d');
    ctx.strokeStyle = '#1b8f91';
    ctx.lineWidth = 2.5;
    let isDrawing = false;
  ...
  async assignTechnician(ticketId, selectId) {
    ...
    // Instantly update UI so user has zero lag
    this.saveTicketsToStorage();
    this.renderTickets();
    this.showToast(`✅ تم إسناد المهمة للفني (${techName}) ونقل التذكرة لمرحلة "قيد التنفيذ" (In Progress)!`);

    // Live sync update to Odoo in background without blocking
  ...
  async completeTicket(ticketId, fileInputId) {
    ...
    if (window._fsTimers && window._fsTimers[tk.id]) {
      clearInterval(window._fsTimers[tk.id]);
    }
    tk.fsTimerRunning = false;
    ...
    const proceed = async (finalPhoto) => {
      ...
      // Live sync update to Odoo in background (Stage 4 = Solved in Helpdesk & Stage 19 = Done in Field Service)
      this.syncTicketUpdateToOdoo(tk).then(() => {
        this.saveTicketsToStorage();
        this.renderTickets();
      }).catch(e => {
  ...
  openSparePartPaymentModal(ticketId) {
    ...
    setTimeout(() => this.initClientSpareSigCanvas(), 120);
  }
  ...
  payForSparePart(ticketId, method) {
    ...
    // Update ticket state
    tk.status = 'تم السداد - بانتظار صرف القطعة من المستودع';
    tk.bgClass = 'badge-info';
...
window.setTechActionPath = function(id, path) { if (window.app) window.app.setTechActionPath(id, path); };
window.requestSparePartFieldService = function(id) { if (window.app) window.app.requestSparePartFieldService(id); };
window.confirmWarehousePickup = function(id) { if (window.app) window.app.confirmWarehousePickup(id); };
window.clearClientSpareSig = function() { if (window.app) window.app.clearClientSpareSig(); };
window.initClientSpareSigCanvas = function() { if (window.app) window.app.initClientSpareSigCanvas(); };
